...
import 'package:custompaintapp/main.dart';
...
class CanvasSideBar extends StatefulWidget {
  ...
  final ValueNotifier<double> imageRotation;
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Slider(
                      value: widget.imageRotation.value,
                      min: 0,
                      max: 360,
                      onChanged: (val) {
                        widget.imageRotation.value = val;
                      },
                    ),
                  ],
                ),
  ...
  void saveFile(Uint8List bytes, String extension) async {
    if (kIsWeb) {
      html.AnchorElement()
        ...
        ..download =
            'CustomCanvas-${DateTime.now().toIso8601String()}.$extension'
        ..style.display = 'none'
        ..click();
    } else {
      await FileSaver.instance.saveFile(
        name: 'CustomCanvas-${DateTime.now().toIso8601String()}.$extension',
        bytes: bytes,
        ext: extension,
