...
class DrawingCanvas extends StatefulWidget {
  ...
  final ValueNotifier<double>? imageRotationListenable;
              ...
              child: RepaintBoundary(
                child: CustomPaint(
                  ...
                  painter: _DrawingCanvasPainter(
                    ...
                    imageRotationListenable: widget.imageRotationListenable,
                  ),
                ),
              ),
            ),
  ...
  void paint(Canvas canvas, Size size) {
    if (backgroundImageListenable != null) {
      ...
      if (backgroundImage != null) {
        ...
        //Rotate the image by using the listenable
        double degrees = 0;
        if (imageRotationListenable != null) {
          degrees = imageRotationListenable!.value;
        }
        double angle = degrees * pi / 180;
        final center = Offset(size.width / 2, size.height / 2);

        canvas.save();
        canvas.translate(center.dx, center.dy); // Move to center
        canvas.rotate(angle); // Rotate
        canvas.translate(
            -imageSize.width / 2, -imageSize.height / 2); // Move back
